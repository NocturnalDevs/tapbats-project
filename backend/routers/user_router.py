...
def save_user(user: UserCreate, inputted_referral_code: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        existing_user = crud.get_user_with_telegram_id(db, telegram_id=user.telegram_id)
        if existing_user:
        ...
        # Handle unique constraint violations (e.g., duplicate telegram_id or referral_code)
        db.rollback()
        logger.error(f"Integrity error saving user: {str(e)}")
